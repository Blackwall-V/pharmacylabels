...
import { parseProductHtml as parseFasaHtml } from "../scrapers/fasa.scraper";
import { parseProductHtml as parseSalcobrandHtml } from "../scrapers/salcobrand.scraper";
import { persistScrapedProduct } from "../pipeline/persist";

const parsers: Record<string, typeof parseFasaHtml> = {
  "farmacias-ahumada": parseFasaHtml,
  salcobrand: parseSalcobrandHtml,
};
...
async function main() {
  ...
  if (!chainSlug || !parsers[chainSlug] || !filePath || !url) {
    console.error(
      `Usage: backfill-from-file.ts --chain=<${Object.keys(parsers).join("|")}> --file=<path> --url=<original-url>`,
    );
    process.exit(1);
  }

  const html = readFileSync(filePath, "utf-8");
  const product = parsers[chainSlug](html, url);
  if (!product) {
